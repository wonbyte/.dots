...
-- Source current file
keymap("n", "<leader>ss", "<cmd>source %<CR>", { desc = "Source current file" })
...
keymap("n", "<leader>ac", function()
  ...
  if enabled == nil then
    enabled = vim.g.completion ~= false
  end
  vim.b.completion = not enabled
  vim.notify("Autocomplete " .. (enabled and "disabled" or "enabled"), vim.log.levels.INFO)
